...
import {
  ...
  postNotification,
  deleteNotification,
} from "../../../features/Notificians/notifcnsSlice";
import AdminAside from "../adminComp/adminAside";

// Komponent və Redux ilə tam uyğun interfeys
...
const AdminNotf: React.FC = () => {
  const [title, setTitle] = useState<string>("");
  const [content, setContent] = useState<string>("");

  const dispatch = useDispatch<AppDispatch>();

  // state.notifications-u birbaşa hədəf tiplə oxuyuruq
  const { notifications, status } = useSelector(
    (state: RootState) => state.notifications
  ) as { notifications: NotificationItem[]; status: string };
  ...
  const handleSendNotification = (): void => {
    ...
    // Obyekt strukturunu dəqiq saxlayırıq (any-siz)
    const notification = {
      title,
      content,
  ...
  const handleDeleteNotification = (id: string | number): void => {
              ...
              <tbody>
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    <tr key={String(notif.id)} className="hover:bg-gray-50">
